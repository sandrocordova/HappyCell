...
class ClienteView(APIView):
    def post(self, request):
        ...
        secuencia = Secuencia.objects.using('clientes').filter(
            SECU_TABLA='CLIENTE', EMPR_CODIGO='8').first()
        ...
        clienteChecking = Cliente.objects.using('clientes').filter(
            CLIE_IDENTIFICACION=clie_identificacion).first()
        ...
        secuencia = Secuencia.objects.using('clientes').filter(
            SECU_TABLA='CLIENTE', EMPR_CODIGO='8').update(SECU_VALOR_ACTUAL=clie_codigo + 1)
    ...
    def put(self, request):
        ...
        ticl_codigo = informacionCliente['TICL_CODIGO']
        clienteChecking = Cliente.objects.using(
            'clientes').filter(CLIE_CODIGO=clie_codigo).first()
        ...
        if validarC['status'] is False:
            return Response({"status": 400, "message": validarC['log'][0]}, status=status.HTTP_400_BAD_REQUEST)
        if ticl_codigo == 'N':
            detalleCliente = ClienteNatural.objects.using(
                'clientes').filter(CLIE_CODIGO=clie_codigo).first()
            validarCN = validarClienteNatural(detalleSCliente)
            if validarCN['status'] is False:
                return Response({"status": 400, "message": validarCN['log'][0]}, status=status.HTTP_400_BAD_REQUEST)
            cambiosCD = actualizarClienteNatural(
                detalleSCliente, detalleCliente)
            tipo = 'Natural'
        if ticl_codigo == 'J':
            detalleCliente = ClienteJuridico.objects.using(
                'clientes').filter(CLIE_CODIGO=clie_codigo).first()
            validarCJ = validarClienteJuridico(detalleSCliente)
            if validarCJ['status'] is False:
                return Response({"status": 400, "message": validarCJ['log'][0]}, status=status.HTTP_400_BAD_REQUEST)
            cambiosCD = actualizarClienteJuridico(
                detalleSCliente, detalleCliente)
            tipo = 'Jurídico'
